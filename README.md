...
| Codex | `AGENTS.md` (protocolo completo inline) | Especificación contra el repo real, QA pegado al código, lectura de repo, testing |
| Google Antigravity | `.agent/rules/uxos-protocolo.md` (always-on) + `.agent/workflows/uxos-*.md` + `GEMINI.md` | QA visual, evidencia de navegación, verificación interactiva |

Las capacidades de navegador dependen de la configuración de cada instalación, no del adaptador: si no hay navegador disponible, el sistema degrada a trabajar sobre capturas provistas y lo registra (regla de `nucleo/sistema.md`).

**Nota sobre agentes constructores (Antigravity, Codex):** sus adaptadores no son enrutadores finos sino protocolos completos con guardarraíl explícito — *este workspace no es un proyecto de software; se investiga, documenta y verifica, no se programa* — porque el instinto por defecto de esas plataformas es construir. La regla always-on de Antigravity y los workflows nativos existen para que el agente sepa exactamente qué hacer en vez de irse a implementar.
...
3b. **Investigaciones paralelas de tres tipos:** auditoría propia, **benchmark de competencia** (patrones dominante/dividido/oportunidad, base Ley de Jakob, sin severidad para competidores) e **investigación con usuarios** (plan/guion/análisis; las sesiones las conduce un humano). Cada frente tiene carpeta propia nombrada por lo que investiga (`10-evidencia/<slug>/`, `20-hallazgos/<slug>/`), hallazgos namespaced (`H-<slug>-<nn>`) y su fila en `ESTADO.md` — varias pueden correr a la vez, incluso desde plataformas distintas, sin colisionar.
4. **Canon citable con límites:** los 8 libros destilados en `nucleo/canon/` con claves `[archivo:concepto]` — Nielsen, Krug, Yablonski, Norman (×2), Walter, Rosenfeld, Crampton Smith — más `wcag-accesibilidad.md` (WCAG 2.2, estándar público) para el barrido de accesibilidad. Cada archivo incluye su "Cuándo NO aplica": un principio usado fuera de su dominio es un hallazgo falso.
